<div align="center">
  <img src="https://raw.vayfou.cn/https://raw.githubusercontent.com/mtvpls/Selene-Build/main/logo.jpg" width="120" alt="Selene" style="margin-bottom:15px;">
  <h1 style="margin:0; color:#2c3e50; font-size:2em;">MoonTVPlus 客户端</h1>
  <p style="color:#666; margin-top:8px;">多平台媒体播放工具 · 版本 v0.6.0</p>
</div>

> 🎬 **Selene** 是以 [MoonTV](https://github.com/MoonTechLab/LunaTV) v100 版本 / [Helios](https://github.com/MoonTechLab/Helios) 为后端的客户端，保证原汁原味的同时，优化了移动端和桌面端操作体验。它基于 **Flutter** 构建，目前支持 Android、iOS、macOS 和 Windows 平台。

<div align="center" style="margin-top:10px;">

![Flutter](https://img.shields.io/badge/Flutter-3.4.3+-02569B?logo=flutter)
![Dart](https://img.shields.io/badge/Dart-3.4.3+-0175C2?logo=dart)
![Android](https://img.shields.io/badge/Android-5.0+(API_21)-3DDC84?logo=android)
![iOS](https://img.shields.io/badge/iOS-13.0+-000000?logo=ios)
![macOS](https://img.shields.io/badge/macOS-11.0+-000000?logo=apple)
![Windows](https://img.shields.io/badge/Windows-10+-0078D6?logo=windows)

</div>

---

### 📱 移动端下载
- **Android 端 (Selene)**
  <br>
  [app-release.apk](https://raw.vayfou.cn/https://github.com/mtvpls/Selene-Build/releases/download/v0.6.0/app-release.apk)

- **iOS 端 (Selene)**
  <br>
  <small>需自签名安装，推荐添加到主屏幕使用：</small>
  <br>
  [selene-0.6.0.ipa](https://raw.vayfou.cn/https://github.com/mtvpls/Selene-Build/releases/download/v0.6.0/selene-0.6.0.ipa)

---

### 💻 桌面端下载
- **Windows 端 (Selene)**
  <br>
  [selene-0.6.0-windows-x64-portable.zip](https://github.com/mtvpls/Selene-Build/releases/download/v0.6.0/selene-0.6.0-windows-x64-portable.zip)

- **Mac 端 (Selene)**
  <br>
  <small>根据芯片架构选择：</small>
  <br>
  - Apple Silicon (ARM 架构)：[selene-0.6.0-macos-arm64.dmg](https://github.com/mtvpls/Selene-Build/releases/download/v0.6.0/selene-0.6.0-macos-arm64.dmg)
  - Intel (AMD64 架构)：[selene-0.6.0-macos-x86_64.dmg](https://github.com/mtvpls/Selene-Build/releases/download/v0.6.0/selene-0.6.0-macos-x86_64.dmg)

---

### 📺 电视端下载
- **Android TV 端 (OrionTV)**
  <br>
  <small>适配电视屏幕尺寸，版本 v2.1.0：</small>
  <br>
  [OrionTV2.1.0.apk](https://raw.vayfou.cn/https://github.com/mtvpls/MoonTVPlus/releases/download/OrionTV适配版2/OrionTV2.1.0.apk)

---

### 📦 其他资源
- **Web 端 (MoonTVPlus)**
  <br>
  <small>即服务器地址，版本 v215.0.0：</small>
  <br>
  [https://tv.vayfou.cn](https://tv.vayfou.cn)
- **影视采集接口测速报告**
  <br>
  <small>每天自动更新：</small>
  <br>
  [https://speed.vayfou.cn](https://speed.vayfou.cn)
- **盘小子**
  <br>
  <small>资源搜索引擎：</small>
  <br>
  [https://pan.xiaozi.cc](https://pan.xiaozi.cc)
